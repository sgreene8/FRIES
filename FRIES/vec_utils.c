...
void collect_procs(dist_vec *vec) {
    ...
    vec_sizes[my_rank] = (int)vec->curr_size;
#ifdef USE_MPI
...
#ifdef USE_MPI
    MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, vec->indices, vec_sizes, disps, MPI_LONG_LONG, MPI_COMM_WORLD);
    MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, vec->values, vec_sizes, disps, mpi_type, MPI_COMM_WORLD);
#endif
    vec->curr_size = tot_size;
}
