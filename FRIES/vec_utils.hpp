...
public:
    ...
    DistVec(size_t size, size_t add_size, uint8_t n_bits,
            unsigned int n_elec, int n_procs, std::vector<uint32_t> rns_common, std::vector<uint32_t> rns_distinct) : values_(1, size), curr_vec_idx_(0), ini_success_(0), ini_fail_(0), max_size_(size), curr_size_(0), occ_orbs_(size, n_elec), adder_(new Adder<el_type>(add_size, n_procs, n_bits, MPI_COMM_WORLD)), n_nonz_(0), indices_(size, CEILING(n_bits, 8)), n_bits_(n_bits), n_dense_(0), nonini_occ_add(0), diag_calc_(nullptr), curr_shift_(nullptr), matr_el_(size), vec_hash_(CEILING(size, 5), rns_distinct), proc_hash_(0, rns_common), min_del_idx_(0), mpi_communicator_(MPI_COMM_WORLD) { }
    
    DistVec(size_t size, Adder<el_type> *adder, uint8_t n_bits,
            unsigned int n_elec, std::vector<uint32_t> rns_common, std::vector<uint32_t> rns_distinct) : values_(1, size), curr_vec_idx_(0), ini_success_(0), ini_fail_(0), max_size_(size), curr_size_(0), occ_orbs_(size, n_elec), adder_(adder), n_nonz_(0), indices_(size, CEILING(n_bits, 8)), n_bits_(n_bits), n_dense_(0), nonini_occ_add(0), diag_calc_(nullptr), curr_shift_(nullptr), matr_el_(size), vec_hash_(CEILING(size, 5), rns_distinct), proc_hash_(0, rns_common), mpi_communicator_(adder->communicator()), min_del_idx_(0) { }
     ...
     */
    DistVec(size_t size, size_t add_size, uint8_t n_bits,
            unsigned int n_elec, int n_procs, std::function<double(const uint8_t *)> diag_fxn, double *shift_ptr, uint8_t n_vecs, std::vector<uint32_t> rns_common, std::vector<uint32_t> rns_distinct) : values_(n_vecs, size), curr_vec_idx_(0), ini_success_(diag_fxn ? size : 0), ini_fail_(diag_fxn ? size : 0), max_size_(size), curr_size_(0), occ_orbs_(size, n_elec), adder_(new Adder<el_type>(add_size, n_procs, n_bits, MPI_COMM_WORLD)), n_nonz_(0), indices_(size, CEILING(n_bits, 8)), n_bits_(n_bits), n_dense_(0), nonini_occ_add(0), diag_calc_(diag_fxn), curr_shift_(shift_ptr), matr_el_(size), vec_hash_(CEILING(size, 5), rns_distinct), proc_hash_(0, rns_common), min_del_idx_(0), mpi_communicator_(MPI_COMM_WORLD) { }
    
    DistVec(size_t size, Adder<el_type> *adder, uint8_t n_bits,
            unsigned int n_elec, std::function<double(const uint8_t *)> diag_fxn, double *shift_ptr, uint8_t n_vecs, std::vector<uint32_t> rns_common, std::vector<uint32_t> rns_distinct) : values_(n_vecs, size), curr_vec_idx_(0), ini_success_(diag_fxn ? size : 0), ini_fail_(diag_fxn ? size : 0), max_size_(size), curr_size_(0), occ_orbs_(size, n_elec), adder_(adder), n_nonz_(0), indices_(size, CEILING(n_bits, 8)), n_bits_(n_bits), n_dense_(0), nonini_occ_add(0), diag_calc_(diag_fxn), curr_shift_(shift_ptr), matr_el_(size), vec_hash_(CEILING(size, 5), rns_distinct), proc_hash_(0, rns_common), mpi_communicator_(adder->communicator()), min_del_idx_(0) { }
