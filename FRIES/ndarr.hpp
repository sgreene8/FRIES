...
public:
     ...
     * Data are copied such that the other rows remain the same before and after this operation
     * \param [in] row      Index of row to remove
     */
     ...
     * Data are copied such that the other rows remain the same before and after this operation
     * \param [in] row      Index at which row will be inserted
     */
    void add_row(size_t row) { 
        ...
        if ((row < rows_) && (row >= 0)) {
            ...
            std::cout << "ERROR: Attempted to add row out of bounds for the matrix: row " << row << "\n";
        }
    }

    /*! \brief Remove column from matrix
     * Data are copied such that other columns remain the same before and after this operation
     * \param [in] col      Index of column to remove
     */
    void remove_col(size_t col) {
        if ((col < cols_) && (col >= 0)) {
            ...
            std::cout << "ERROR: Attempted to remove col out of bounds for the matrix: col " << col << "\n";
            exit(0);
        }
    }
        
    /*! \brief Increase number of columns in the matrix
     * Data are copied such that the first n_keep[i] elements in each row remain the same before and after this operation
    ...
    /*! \brief Increase number of columns in the matrix
     * Data are copied such that the first n_keep elements in each row remain the same before and after this operation
     * \param [in] new_col      Desired number of columns in the enlarged matrix
    ...
    */
    ...
    /*!
    * \brief Prints the matrix to standard out
    */
    void print() {
        std::cout << "[ ";
...
/*! \brief A class for storing 4-D arrays
 * \tparam arr_type The type of elements to be stored in the 4-D array
 */
...
public:
    ...
    , len4_(len4)
    {
        data_ = (arr_type *)malloc(sizeof(arr_type) * len1 * len2 * len3 * len4);
     ...
     */
    arr_type& operator() (size_t i1, size_t i2, size_t i3, size_t i4) {
        return data_[i1 * len2_ * len3_ * len4_ + i2 * len3_ * len4_ + i3 * len4_ + i4];
    }
    
    arr_type  operator() (size_t i1, size_t i2, size_t i3, size_t i4) const {
        return data_[i1 * len2_ * len3_ * len4_ + i2 * len3_ * len4_ + i3 * len4_ + i4];
    }
    ...
    /*! \brief Get indices of nonzero elements
     * \return Matrix containing indices of nonzero elements
    */
    Matrix<size_t> nonzero() {
        size_t vec_size = (len1_ * len2_ * len3_* len4_) / 8;
        Matrix<size_t> mat_out(vec_size, 4);

        size_t elem = 0; 
        for (size_t i1 = 0; i1 < len1_; i1++) {
            for (size_t i2 = 0; i2 < len2_; i2++) {
                for (size_t i3 = 0; i3 < len3_; i3++) {
                    for (size_t i4 = 0; i4 < len4_; i4++) {
                        if ( (*this)(i1, i2, i3, i4) != false ) {
                            if (elem >= (vec_size - 1)) {
    ...
    /*!
    * \brief Prints the FourDArr to standard out
    */
    void print() { 
        std::cout << "\n[ ";
        for (size_t i1 = 0; i1 < len1_; i1++) {
    ...
    /*! \brief Get elements at specified indices
     * \param [in] indices    Indices of elements to retrieve
     * \return Vector of elements at specified indices
    */
    std::vector<arr_type> get_elements(Matrix<size_t> &indices) {
        if (indices.cols() != 4) {
            std::cout << "ERROR: get_elements() requires a matrix with 4 columns\n";
            exit(0);
        }
        std::vector<arr_type> output(indices.rows());

        for (size_t i = 0; i < indices.rows(); i++) {
            output[i] = (*this)(indices[i][0], indices[i][1], indices[i][2], indices[i][3]);
        }
        return output;
    }

    /*! \brief Set elements at specified indices to specified values
     * \param [in] indices    Indices of elements to set
     * \param [in] values     Values to set elements to
    */
    void set_elements(Matrix<size_t> &indices, std::vector<arr_type> values) {
        for (int i=0; i<(int)indices.rows(); i++) {
            (*this)(indices[i][0], indices[i][1], indices[i][2], indices[i][3]) = values[i];
        }
    }
    
    /*! \brief Zero all elements in the array */
    void zero() {
        std::fill(data_, data_ + len1_ * len2_ * len3_ * len4_, 0);
    }

private:
    size_t len1_, len2_, len3_, len4_; ///< Dimensions of the array
    arr_type *data_; ///< The data stored in the array
};

template<> class FourDArr<bool> {
...
public:
    ...
    /*!
     * \brief Constructor for FourDArr.
    ...
    */
    ...
    /*!
     * \brief Retrieves the indices and values of nonzero elements.
     * \return A Matrix containing indices and values of nonzero elements.
     */
    Matrix<size_t> nonzero_elems() {
        size_t vec_size = (len1_ * len2_ * len3_ * len4_) / 8;
        Matrix<size_t> mat_out(vec_size, 5);
                ...
                for (size_t i3=0; i3 < len3_; i3++) {
                    for (size_t i4=0; i4 < len4_; i4++) {
                        if ( (*this)(i1, i2, i3, i4) != 0 ) {
                            ...
                            mat_out[elem][2] = i3;
                            mat_out[elem][3] = i4;
                            mat_out[elem][4] = (*this)(i1, i2, i3, i4);
                            elem ++;
                        }
                    }
                }
