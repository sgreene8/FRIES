...
struct MyArgs : public argparse::Args {
    ...
    std::shared_ptr<std::string> &load_dir = kwarg("load_dir", "Directory from which to load checkpoint files from a previous FCIQMC calculation (in binary format, see documentation for DistVec::save() and DistVec::load())");
...
int main(int argc, char * argv[]) {
    ...
    try {
        ...
        DistVec<double> sol_vec(max_n_dets, spawn_length, n_orb * 2, n_elec_unf, n_procs, diag_shortcut, 1, proc_scrambler, vec_scrambler);
