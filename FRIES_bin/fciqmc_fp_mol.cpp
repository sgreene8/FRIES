...
int main(int argc, const char * argv[]) {
    ...
    double target_norm = (double)program.get<float>("target");
    ...
    // Solution vector
    unsigned int spawn_length = target_norm / n_procs / n_procs * 2;
...
# pragma mark Set up trial vector
    ...
    if (proc_rank == hf_proc) {
        ...
        fprintf(param_f, "FCIQMC calculation\nHF path: %s\nepsilon (imaginary time step): %lf\nTarget number of walkers %lf\nInitiator threshold: %.2f\n", hf_path, eps, target_norm, init_thresh);
        if (load_dir) {
