...
static const char *const usage[] = {
    "symm_arnoldi_mol [options] [[--] args]",
    "symm_arnoldi_mol [options]",
    NULL,
};
...
int main(int argc, const char * argv[]) {
    ...
    struct argparse_option options[] = {
        ...
        OPT_STRING('v', "ini_vecs", &ini_path, "Prefix for files containing the vectors with which to initialize the calculation (unless load_dir is provided). Files must have names <trial_vecs>dets<xx> and <trial_vecs>vals<xx>, where xx is a 2-digit number ranging from 0 to (n_states - 1), and be text files."),
        OPT_INTEGER('k', "num_states", &n_states, "Number of states whose energies will be computed."),
    ...
    if (!ini_path) {
        fprintf(stderr, "Error: path to initial vectors was not specified.\n");
        return 0;
    }
    if (n_states < 2) {
